...
void Plus(Local<Value> &obj) {
  ...
  for (unsigned int i = 0; i < properties->Length(); i++) {
    Local<String> key =
        Nan::To<String>(Nan::Get(properties, i).ToLocalChecked())
            .ToLocalChecked();
    Local<Value> val1;
    Local<Value> val2;
...
NAN_METHOD(PlusOne) {
  Nan::JSON json;
  Nan::MaybeLocal<Value> parsed =
      json.Parse(Nan::To<String>(info[0]).ToLocalChecked());
  ...
  info.GetReturnValue().Set(
      json.Stringify(Nan::To<Object>(ret).ToLocalChecked()).ToLocalChecked());
}

NAN_MODULE_INIT(Init) { Nan::Export(target, "plusOne", PlusOne); }
