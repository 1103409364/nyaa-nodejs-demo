...
struct ScopeLock {
  ...
  ScopeLock(uv_mutex_t *handle) : handle(handle) { uv_mutex_lock(handle); }

  ~ScopeLock() { uv_mutex_unlock(handle); }
};
...
NAN_METHOD(Sort) {
  ...
  for (uint32_t i = 0; i < array->Length(); i++) {
    ...
    orig.push_back(
        Nan::To<uint32_t>(Nan::Get(array, i).ToLocalChecked()).FromJust());
  }
...
NAN_MODULE_INIT(Init) { Nan::Export(target, "sort", Sort); }
