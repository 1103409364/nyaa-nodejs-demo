...
public:
  DownloadWorker(string uri, Nan::Callback *callback, Nan::Callback *processor)
      : AsyncProgressWorker(callback), processor(processor), uri(uri) {}

  ~DownloadWorker() { delete processor; }
  ...
  void HandleProgressCallback(const char *data, size_t count) {
    ...
    Local<Value> argv[] = {Nan::CopyBuffer(data, count).ToLocalChecked()};
...
static void AtExit(void *arg) { minihttp::StopNetwork(); }
